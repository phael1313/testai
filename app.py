...
<body>
    ...
    <div class="controls">
        ...
        <input type="text" id="responsavel" placeholder="Digite o nome"><br>
        <label for="data">Data do Teste:</label><br>
        <input type="date" id="data"><br>
    </div>
    <div id="checkboxes">
        <!-- CHECKBOXES GERADOS AQUI -->
    </div>
    <div class="section">
        <button onclick="gerarRelatorioAjustes()">📄 Gerar Relatório de Ajustes</button>
        <div id="ajustes" style="margin-top: 20px;"></div>
    </div>
    <script>
        function gerarRelatorioAjustes() {
            const checkboxes = document.querySelectorAll('input[type=checkbox]');
            let ajustes = '<h3>Relatório de Ajustes</h3><ul>';
            checkboxes.forEach((cb, i) => {
                if (!cb.checked) {
                    const label = cb.nextSibling.textContent || "Item " + (i + 1);
                    ajustes += `<li><strong>${label}</strong>: Este item ainda não foi testado.</li>`;
                }
            });
            ajustes += '</ul>';
